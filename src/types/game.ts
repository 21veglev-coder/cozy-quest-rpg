...
export interface EnemyAbility {
  name: string;
  icon: string;
  chance: number; // 0-100, checked each turn
  type: 'attack' | 'heal' | 'buff' | 'debuff';
  damage?: number; // flat or multiplier
...
export const ENEMY_ABILITIES: Record<string, EnemyAbility> = {
  ...
  heal_self: { name: 'Regeneráció', icon: '💚', chance: 40, type: 'heal', heal: 25, condition: 'low_hp', description: 'Az ellenség gyógyul!' },
  war_cry: { name: 'Harci Üvöltés', icon: '📣', chance: 20, type: 'buff', selfBuff: { atk: 5, def: 3, turns: 2 }, condition: 'always', description: 'Harci üvöltés zeng!' },
  shield_up: { name: 'Védekezés', icon: '🛡️', chance: 30, type: 'buff', selfBuff: { def: 8, turns: 2 }, condition: 'always', description: 'Védekezésbe húzódik!' },
  dark_blast: { name: 'Sötét Robbanás', icon: '💀', chance: 35, type: 'attack', damageMultiplier: 2.0, condition: 'always', description: 'Sötét energia robban!' },
  vampiric_bite: { name: 'Vámpír Harapás', icon: '🧛', chance: 40, type: 'attack', damageMultiplier: 1.5, heal: 15, condition: 'always', description: 'Vért szív!' },
...
export function getEnemyAction(enemy: EnemyDef, currentHp: number): EnemyAbility | null {
  ...
  const hpPercent = currentHp / enemy.hp;
  const eligible = enemy.abilities.filter(a => {
    if (a.condition === 'low_hp' && hpPercent > 0.3) return false;
    return true;
  });
  for (const ability of eligible.sort((a, b) => b.chance - a.chance)) {
    if (Math.random() * 100 < ability.chance) return ability;
  }
  return null;
}
...
export const ENEMIES: EnemyDef[] = [
  ...
  { name: 'Farkas', icon: '🐺', level: 2, hp: 45, atk: 8, def: 3, spd: 12, xpReward: 25, goldReward: 8, locationType: 'zone', minZoneLevel: 1, world: 1, abilities: [ENEMY_ABILITIES.enrage] },
  { name: 'Pók Királyné', icon: '🕷️', level: 3, hp: 60, atk: 10, def: 5, spd: 6, xpReward: 40, goldReward: 15, locationType: 'zone', minZoneLevel: 3, world: 1, abilities: [ENEMY_ABILITIES.poison_spit] },
  { name: 'Bandita', icon: '🥷', level: 2, hp: 40, atk: 9, def: 4, spd: 10, xpReward: 20, goldReward: 10, locationType: 'zone', minZoneLevel: 1, world: 1, abilities: [ENEMY_ABILITIES.heavy_strike] },
  { name: 'Mérgező Kígyó', icon: '🐍', level: 3, hp: 35, atk: 12, def: 2, spd: 15, xpReward: 30, goldReward: 12, locationType: 'zone', minZoneLevel: 2, world: 1, abilities: [ENEMY_ABILITIES.poison_spit, ENEMY_ABILITIES.enrage] },
  { name: 'Csontváz Harcos', icon: '💀', level: 5, hp: 110, atk: 18, def: 12, spd: 9, xpReward: 70, goldReward: 25, locationType: 'zone', minZoneLevel: 5, world: 1, abilities: [ENEMY_ABILITIES.heavy_strike, ENEMY_ABILITIES.shield_up] },
  { name: 'Medveember', icon: '🐻', level: 4, hp: 95, atk: 13, def: 10, spd: 6, xpReward: 50, goldReward: 18, locationType: 'zone', minZoneLevel: 3, world: 1, abilities: [ENEMY_ABILITIES.war_cry, ENEMY_ABILITIES.enrage] },
  { name: 'Tűz Elementál', icon: '🔥', level: 7, hp: 200, atk: 28, def: 10, spd: 12, xpReward: 130, goldReward: 45, locationType: 'zone', minZoneLevel: 7, world: 1, abilities: [ENEMY_ABILITIES.fire_breath, ENEMY_ABILITIES.enrage] },
  { name: 'Szellem', icon: '👻', level: 6, hp: 100, atk: 22, def: 4, spd: 20, xpReward: 90, goldReward: 30, locationType: 'zone', minZoneLevel: 5, world: 1, abilities: [ENEMY_ABILITIES.dark_blast] },
  { name: 'Orc Harcvezér', icon: '👹', level: 6, hp: 150, atk: 22, def: 14, spd: 9, xpReward: 100, goldReward: 35, locationType: 'zone', minZoneLevel: 5, world: 1, abilities: [ENEMY_ABILITIES.war_cry, ENEMY_ABILITIES.heavy_strike, ENEMY_ABILITIES.enrage] },
  { name: 'Jég Elem', icon: '❄️', level: 8, hp: 220, atk: 26, def: 14, spd: 11, xpReward: 150, goldReward: 55, locationType: 'zone', minZoneLevel: 7, world: 1, abilities: [ENEMY_ABILITIES.ice_blast, ENEMY_ABILITIES.shield_up] },
  { name: 'Sötét Lovag', icon: '🏇', level: 9, hp: 280, atk: 32, def: 20, spd: 13, xpReward: 180, goldReward: 65, locationType: 'zone', minZoneLevel: 8, world: 1, abilities: [ENEMY_ABILITIES.heavy_strike, ENEMY_ABILITIES.shield_up, ENEMY_ABILITIES.enrage] },
  // World 1 - Dungeon
  { name: 'Sötét Varázsló', icon: '🧙‍♂️', level: 4, hp: 120, atk: 22, def: 8, spd: 11, xpReward: 100, goldReward: 40, locationType: 'dungeon', minZoneLevel: 4, world: 1, abilities: [ENEMY_ABILITIES.dark_blast, ENEMY_ABILITIES.heal_self] },
  { name: 'Kőgolem', icon: '🗿', level: 5, hp: 250, atk: 18, def: 26, spd: 4, xpReward: 120, goldReward: 35, locationType: 'dungeon', minZoneLevel: 4, world: 1, abilities: [ENEMY_ABILITIES.shield_up, ENEMY_ABILITIES.heavy_strike] },
  { name: 'Vámpír Gróf', icon: '🧛', level: 6, hp: 170, atk: 26, def: 14, spd: 16, xpReward: 150, goldReward: 55, locationType: 'dungeon', minZoneLevel: 6, world: 1, abilities: [ENEMY_ABILITIES.vampiric_bite, ENEMY_ABILITIES.enrage] },
  { name: 'Múmia Úr', icon: '🧟', level: 5, hp: 150, atk: 20, def: 18, spd: 6, xpReward: 110, goldReward: 45, locationType: 'dungeon', minZoneLevel: 4, world: 1, abilities: [ENEMY_ABILITIES.poison_spit, ENEMY_ABILITIES.heal_self] },
  { name: 'Méregpók Anya', icon: '🕸️', level: 6, hp: 140, atk: 28, def: 10, spd: 14, xpReward: 130, goldReward: 50, locationType: 'dungeon', minZoneLevel: 5, world: 1, abilities: [ENEMY_ABILITIES.poison_spit, ENEMY_ABILITIES.enrage] },
  { name: 'Lich Király', icon: '👑', level: 8, hp: 350, atk: 32, def: 18, spd: 10, xpReward: 280, goldReward: 100, locationType: 'dungeon', minZoneLevel: 8, world: 1, abilities: [ENEMY_ABILITIES.dark_blast, ENEMY_ABILITIES.heal_self, ENEMY_ABILITIES.boss_enrage] },
  { name: 'Démon Vezér', icon: '😈', level: 9, hp: 420, atk: 38, def: 22, spd: 12, xpReward: 350, goldReward: 130, locationType: 'dungeon', minZoneLevel: 8, world: 1, abilities: [ENEMY_ABILITIES.fire_breath, ENEMY_ABILITIES.war_cry, ENEMY_ABILITIES.boss_enrage] },
  // World 1 - Raid
  { name: 'Ősi Sárkány', icon: '🐉', level: 10, hp: 800, atk: 50, def: 30, spd: 14, xpReward: 700, goldReward: 300, locationType: 'raid', minZoneLevel: 8, world: 1, abilities: [ENEMY_ABILITIES.fire_breath, ENEMY_ABILITIES.war_cry, ENEMY_ABILITIES.shield_up, ENEMY_ABILITIES.boss_enrage] },
